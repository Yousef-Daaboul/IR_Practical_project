...
from constants import query_pickle_folder_path, clinical_queries_vectorizer_file, clinical_queries_tfidf_matrix_file, \
    limit_docs, clinical_titles_vectorizer_file, clinical_titles_tfidf_matrix_file
...
def fetch_docs_titles():
    ...
    if limit_docs:
        query = 'SELECT title FROM clinical_trials LIMIT ?'
        docs_titles = cursor.execute(query, (limit_docs,)).fetchall()
    else:
        docs_titles = cursor.execute('SELECT title FROM clinical_trials').fetchall()
...
def fetch_queries():
    ...
    db_queries = cursor.execute('SELECT disease, gene, demographic, other FROM clinical_queries').fetchall()

    merged_queries = []
    for disease, gene, demographic, other in db_queries:
        parts = [disease, gene, demographic]
        if other and other.lower() not in {'none', 'null'}:
            parts.append(other)
...
def queries_train():
    ...
    print(f'Start Queries Indexing... (at {datetime.datetime.now()})')
    ...
    with open(clinical_queries_vectorizer_file, "wb") as f:
        pickle.dump(vectorizer, f)

    with open(clinical_queries_tfidf_matrix_file, "wb") as f:
...
def titles_train():
    ...
    with open(clinical_titles_vectorizer_file, "wb") as f:
        pickle.dump(vectorizer, f)

    with open(clinical_titles_tfidf_matrix_file, "wb") as f:
        pickle.dump(tfidf_matrix, f)
